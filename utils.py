...
def _topological_loop_horiz(theta, batch_sizes):
    new = theta.new
...
adjacency_mat = [(-1, -1), (-1, 0), (0, -1)]


def backtrack_adjacency(trellis_batch, heights, widths, adjacency):
    paths = []
    for b in range(trellis_batch.shape[0]):
        ...
        while True:
            ...
            if j == 0:
                ...
                parent_coords = [(max(i+p_i, 0), max(j+p_j, 0)) for p_i, p_j in adjacency]
                parents = torch.cat([trellis[p_i, p_j][None] for p_i, p_j in parent_coords], dim=0)
                parent_idx = torch.argmin(parents)
                i, j = parent_coords[parent_idx]
...
def backtrack_horiz(trellis_batch, heights, widths):
    paths = []
    for b in range(trellis_batch.shape[0]):
        trellis = trellis_batch[b]
    ...
    for b in range(trellis_batch.shape[0]):
        ...
        while True:
            ...
            if j == 0:
                ...
                parents = trellis[i-1, max(j-1, 0):j+1]
                parent_idx = torch.argmin(parents)
                i -= 1
                j -= 1 if parent_idx == 0 else 0
